...
function updateColumns()
{
	...
	xmlHttp.open("GET","index2.php?option=com_rsform&task=plugin&plugin_task=mappings.getColumns&tableName="+currentTable,true);
	xmlHttp.send(null);
}
function saveMapping(formId)
{
	...
	xmlHttp.onreadystatechange=function()
	{
		...
		xmlHttp.open("GET","index2.php?option=com_rsform&task=plugin&plugin_task=mappings.saveMapping&ComponentId="+currentComponent+"&MappingTable="+currentTable+"&MappingColumn="+currentColumn+"&FormId="+formId,true);
	xmlHttp.send(null);
...
function deleteMapping(mappingId,formId)
{
	...
	xmlHttp.open("GET","index2.php?option=com_rsform&task=plugin&plugin_task=mappings.deleteMapping&MappingId="+mappingId+"&FormId="+formId,true);
	xmlHttp.send(null);
}

function exportProcess(post, start, limit, total){
	xml=buildXmlHttp();
	xml.onreadystatechange=function()
    {
		if(xml.readyState==4)
		{
			...
			else
			{
				...
				start = start + limit;
				exportProcess(post, start, limit, total);
			}
		}
    }
		
	xml.open('GET','index.php?option=com_rsform&task=submissions.export.process&post=' + post + '&start=' + start + '&limit=' + limit + '&randomTime=' + Math.random(),true);
	xml.send(null);
}
